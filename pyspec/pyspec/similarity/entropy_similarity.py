...
from pyspec.similarity.util import SpecTypes, _get_spectrum_peak_penalty, _transform_spectrum


def _match_spec(spec_a, spec_b, ms2_ppm=None, ms2_da=None) -> [[]]:
    """
...
def _centroid_spec(spec: np.ndarray, ms2_ppm=None, ms2_da=0.05):
    """
...
def _normalize_spec(spec, ms2_ppm=None, ms2_da=0.05):
    """
    If both ms2_ppm and ms2_da is defined, ms2_da will be used.
    """
    # Remove intensity==0
    ...
    if spec.shape[0] > 0:
        ...
        # Centroid spectrum
        spec = _centroid_spec(spec, ms2_ppm, ms2_da)
...
def get_merged_spectrum(query, reference,
                        ms2_ppm=50, ms2_da=None, need_normalize=True) -> float:
    spec_query = _normalize_spec(query, ms2_ppm=ms2_ppm, ms2_da=ms2_da)
    spec_reference = _normalize_spec(reference, ms2_ppm=ms2_ppm, ms2_da=ms2_da)

    # Calculate distance
    spec_matched = _match_spec(
        spec_a=spec_query, spec_b=spec_reference, ms2_ppm=ms2_ppm, ms2_da=ms2_da)
    merged = spec_matched[:, 1] + spec_matched[:, 2]

    return list(zip(spec_matched[:, 0], merged))
...
def normalize_distance(dist, dist_range):
    if dist_range[1] == np.inf:
        ...
        return (dist - dist_range[0]) / (dist_range[1] - dist_range[0])
...
def entropy_similarity(spec_query: np.ndarray, spec_reference: np.ndarray, ms2_tolerance: float,
                       precursor_cutoff: float = None, normalize_result=True):
    if precursor_cutoff is not None:
        spec_query = spec_query[spec_query[:, 0] < precursor_cutoff - 0.5]
        spec_reference = spec_reference[spec_reference[:, 0] < precursor_cutoff - 0.5]

    spec_query = _normalize_spec(spec_query, ms2_da=ms2_tolerance)
    spec_reference = _normalize_spec(spec_reference, ms2_da=ms2_tolerance)

    spec_matched = _match_spec(spec_a=spec_query, spec_b=spec_reference, ms2_da=ms2_tolerance)
    dist = entropy_distance(spec_matched[:, 1], spec_matched[:, 2])

    # Normalize result to [0,1]
    if normalize_result:
        dist_range = [0, np.log(4)]
        dist = normalize_distance(dist, dist_range)
    return 1 - dist


def entropy_similarity_adaptor(s: SpecTypes, lib: SpecTypes, ms2_tolerance: float,
                               s_precursor: float = None, lib_precursor: float = None, precursor_cutoff: float = None):
    if type(s) == MSMSSpectrum and type(lib) == MSMSSpectrum:
        precursor_s = s.precursors[0][0]
        precursor_lib = lib.precursors[0][0]
    ...
    if precursor_cutoff is None:
        if precursor_s is None:
            if precursor_lib is not None:
                precursor_cutoff = precursor_lib
        else:
            if precursor_lib is None:
                precursor_cutoff = precursor_s
            else:
                precursor_cutoff = min(precursor_s, precursor_lib)

    spec_s = _transform_spectrum(s)
    spec_lib = _transform_spectrum(lib)

    return entropy_similarity(spec_s, spec_lib, ms2_tolerance=ms2_tolerance, precursor_cutoff=precursor_cutoff)
